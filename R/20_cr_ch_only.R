...
result_df <- cr_ch_model(c(crch_centre - crch_window/2, crch_centre + crch_window/2), c(hr_before_aki, hr_after_aki)) %>%
  pivot_longer(
  ...
  mutate(
    heatmap = if_else(
      heatmap == "neg",
      "No AKI",
      paste0("T_AKI in  ", hr_before_aki, "-", hr_after_aki, "hrs after cr_ch"))
  )

heatmap_all <- logit_df %>%
  # filter(is.na(del_t_aki_hr) | del_t_aki_hr > hr_before_aki) %>%
  mutate(
    heatmap = case_when(  # TODO change to factor and make easier to use
      is.na(del_t_aki_hr)          ~ "No AKI",
      del_t_aki_hr > hr_after_aki  ~ paste0("T_AKI in ", hr_after_aki, "+hrs after cr_ch"),
      del_t_aki_hr > hr_before_aki ~ paste0("T_AKI in  ", hr_before_aki, "-", hr_after_aki, "hrs after cr_ch"),
      TRUE                      ~ NA_character_
    ),
  )
...
ggplot(heatmap_ts, aes(x = del_t_ch_hr, y = del_cr)) +
