...
screening_log$screen_out <- rbind(
  ...
  distinct() %>%
  group_by(`UR number`) %>% 
  top_n(-1, `Date first screened`)
  # Consider not removing, then grouping by neither, arrange by date, then fill up
...
test <- screening_log$full %>% 
  ...
  mutate(
    ...
    # Admission  = if_else(is.na(Admission), Total_rows, Admission),
  ...
  select(`UR number`, Event, Dates_screened, `Date first screened`, Age, Admission, Total_admissions, Total_rows, duplicates) %>% 
  filter(duplicates || is.na(duplicates))
 
# Need handling of L, LT or neither

nrow(screening_log$full)
nrow(screening_log$screen_in) + nrow(screening_log$screen_neither)
