...
test <- screening_log$full %>% 
  ...
  group_by(`UR number`, Pt_Study_no_crch, Pt_Study_no_olig) %>% 
  arrange(-Total_rows, `UR number`, Admission) %>% 
  fill(.direction = "updown") %>% 
  # select(`UR number`, Age, Admission, Total_admissions, Total_rows, duplicates) %>% 
  filter(duplicates || is.na(duplicates))
 


nrow(screening_log$full)
nrow(screening_log$screen_in) + nrow(xlsx_data$screen_out$neither_cr_ol)
